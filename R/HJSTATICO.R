...
#' \itemize{
#' \item *X:*	The standardized matrix of x, resulting from using the Prep.Tables() function from the 'StatisMix' library, with the preprocessing argument indicating the type of standardization to be applied.
...
#' }
#'
#' @author Mariela González-Narváez, Ph.D., María José Fernández Gomez, Susana Mendes, Purificación Galindo-Villardón, Ph.D., Gema Zambrano Zambrano, Ysaí Ronquillo Mora, George Acosta Chong
#'
...
HJSTATICO=function(x,y,condition,bplot,preprocessing,compx,compy) {
  ...
  prep=StatisMix::Prep.Tables(x=x,y=y,condition=condition,preprocessing=tp)
  X=prep$X
  Y=prep$Y
  index=prep$index
  #COINERTIA
